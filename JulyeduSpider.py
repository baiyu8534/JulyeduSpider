...
# 现在也行吧，错误的进去的破解步骤，就直接退出就行，从新请求，就是多了步
...
class JulyeduSpider(object):
    ...
    def __init__(self):
        ...
        self.collection = self.client["Julyedu"]["test"]
        # 用来记录请求json解析失败的网页url，可以手动解析
        self.error_json_url = []

        self.__temp_page_url_queue = Queue()
        self.__temp_page_html_queue = Queue()
        self.__info_page_url_queue = Queue()
        self.__info_page_html_queue = Queue()
    ...
    def request_temp_page(self):
        ...
        while self.__is_running.isSet():
            ...
            self.__temp_page_html_queue.put({"html_str": html_str, "item": item, "temp_url": temp_url})

            self.__temp_page_url_queue.task_done()

    def parse_temp_page(self):
        """
        解析json，判断是否是验证码
        :return:
        """
        while self.__is_running:
            data = self.__temp_page_html_queue.get()
            ...
            if len(json_data_str) == 0:
                ...
                if IS_NOT_ID_CODE.isSet():
                    ...
                    self.__temp_page_html_queue.task_done()
                    return
                else:
    ...
    def request_info_page(self):
        ...
        while self.__is_running.isSet():
            ...
            # print("拿到了网页")
            self.__info_page_html_queue.put({"html_str": html_str, "item": item, "info_url": url})
            self.__info_page_url_queue.task_done()

    def parse_info_page(self):
        while self.__is_running.isSet():
            data = self.__info_page_html_queue.get()
            html_str = data["html_str"]
            item = data["item"]
            url = data["info_url"]
            ...
            if len(json_data_str) == 0:
                ...
                if IS_NOT_ID_CODE.isSet():
                    ...
                    self.__info_page_html_queue.task_done()
                    return
                else:
                    IS_NOT_ID_CODE.wait()
                    self.__info_page_url_queue.put({"url": url, "item": item})
    ...
    def run(self):
        ...
        # 2.解析临时网页
        for i in range(4):
            t_parse_temp_page = threading.Thread(target=self.parse_temp_page)
            thread_list.append(t_parse_temp_page)
        # 3.获取题目详情网页
        for i in range(4):
        ...
        # 4.解析题目详情网页
        for i in range(4):
            t_parse_info_page = threading.Thread(target=self.parse_info_page)
            thread_list.append(t_parse_info_page)
        # 5.保存数据
        for i in range(16):
        ...
        for q in [self.__temp_page_html_queue,
                  self.__temp_page_url_queue,
                  self.__info_page_url_queue,
                  self.__info_page_html_queue,
