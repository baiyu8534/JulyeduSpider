...
class JulyeduSpider(object):
    ...
    def __init__(self):
        ...
        self.__id_code_url_queue = Queue()
        ...
        while self.__is_running.isSet():
            ...
            if len(json_data_str) == 0:
                ...
                if IS_NOT_ID_CODE.isSet():
                    ...
                    self.__id_code_url_queue.put(temp_url)
        ...
        while self.__is_running.isSet():
            ...
            if len(json_data_str) == 0:
                ...
                if IS_NOT_ID_CODE.isSet():
                    ...
                    self.__id_code_url_queue.put(url)
    ...
    def break_id_code(self):
        while self.__is_running.isSet():
            url = self.__id_code_url_queue.get()
            ...
            IS_NOT_ID_CODE.set()
            time.sleep(random.choice([0.2, 0.5, 0.7, 0.9]))
            self.__id_code_url_queue.task_done()
            if not IS_NOT_ID_CODE.isSet():
                IS_NOT_ID_CODE.set()
    ...
    def run(self):
        ...
        t_break_id_code = threading.Thread(target=self.break_id_code)
        thread_list.append(t_break_id_code)
        # 5.保存数据
        for i in range(16):
